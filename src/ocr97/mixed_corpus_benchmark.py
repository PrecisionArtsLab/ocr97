...
def _select_cases(manifest: Mapping[str, Any], *, ids: Optional[Iterable[str]] = None, limit: int = 0) -> Dict[str, Any]:
    ...
    if wanted:
        ...
        ordered = [
            case
            for case in cases
            if (str(case.get("id") or "") in wanted_set) or (str(case.get("source_case_id") or "") in wanted_set)
        ]
    else:
        ordered = cases
    if int(limit or 0) > 0:
        ...
        for run_cfg in self.runs:
            ...
            try:
                runner = MixedCorpusBenchmark(
                    ...
                    focus_ids=_split_csv(run_cfg.get("focus_ids") or ""),
...
class MixedCorpusBenchmark:
    ...
    def _write_report(self, state: Mapping[str, Any]) -> None:
        lines = [
            ...
            "",
            "| Step | Status | Cases | Avg score | Perfect | Below 75 | Avg latency ms | Max latency ms | Seconds |",
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    ...
    def _plan(self, manifest: Mapping[str, Any]) -> List[Dict[str, Any]]:
        ...
        focus = _select_cases(manifest, ids=self.focus_ids, limit=self.focus_limit)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--focus-ids", default=",".join(DEFAULT_FOCUS_IDS), help="Comma-separated case ids for heavy focus lane.")
    ...
    runner = MixedCorpusBenchmark(
        ...
        focus_ids=_split_csv(args.focus_ids),
